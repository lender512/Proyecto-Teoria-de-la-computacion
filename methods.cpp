...
NFA reverse(DFA input){
    ...
    //if there is only one final state:
    if (input.final_states.size() == 1){
        ...
        //-1 is the start state for DFA if count of final states is greather than 0:
    else{
...
DFA determinization(NFA input){
    auto tempm = input.closure(input.initial_state); //temp
    vector<transition> transitions = input.transitions; //old
    set<int> new_initial(tempm.begin(),tempm.end()); //initial - not renamed
    set<set<int>> passed = {new_initial}; // {{1,2},{2,3}}
    set<set<int>> done = {};
    int postinitial = 0;
    ...
    //Algorythim
    do{
        auto ceb = passed; //temp {1,2}
        for (auto i = ceb.begin();i!=ceb.end();advance(i,1)){
            auto temp = *i; //temp con el valor de ceb
            if (done.find(temp) == done.end()){
                //0
                done.insert(temp);
                set<int> temp2 = {}; //destinies of each state
                for (auto i2 = temp.begin();i2!=temp.end();advance(i2,1)) {
                    ...
                    for (auto l = temp2.begin(); l != temp2.end(); advance(l, 1)) { //closure of each destiny of temp2
                        auto temp3 = input.closure(*l);
                        for (auto k = temp3.begin(); k != temp3.end(); advance(k, 1)) {
                            temp2.insert(*k); //insert each element of temp3 on temp2
                        }
                    }
                }
                //set
                vector<set<int>> temp5 = {temp,temp2}; //{origin - destiny}
                newtransitions.push_back(temp5); //temp2 could be empty
                passed.insert(temp2);
                ...
                temp5 = {temp,temp2};
                newtransitions.push_back(temp5);
                passed.insert(temp2);
    ...
    for (auto it = passed.begin();it!=passed.end();advance(it,1)){
        ...
        bool final = false;
        set<int> temp4 = *it; //each state of passed (or done)
        dic_new[temp4] = i;
